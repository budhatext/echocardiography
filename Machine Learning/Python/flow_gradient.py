...
sql_query = pd.read_sql_query('SELECT * FROM SCHEMA.NLPFORFG_PROSPECTIVE WHERE FLOW_GRADIENT_OUTCOMES IS NOT NULL ',conn)
df = pd.DataFrame(sql_query, columns=['Findings_Value','FLOW_GRADIENT_OUTCOMES'])
print(df)
...
plot_confusion_matrix(text_clf,X_test,y_test,display_labels =["0=NA","1=PLFLG","2=LFLG"])
