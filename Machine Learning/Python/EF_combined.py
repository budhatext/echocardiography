...
conn = pyodbc.connect('Driver={SQL Server};'
                      ...
                      'user =HHVDBCORT01\\TMHBXB78;'
                      'Trusted_Connection=yes;')
sql_query = pd.read_sql_query('SELECT * FROM NLPFOREF_BANDS_PROSPECTIVE',conn)
...
X=df['Findings_Value']
y=df['FINAL_EF']
df.to_excel (r'I:\BUDHA\C3PH ECHO\Python pipeline\export_dataframe_EF_bands.xlsx', index = False, header=True)
count_vect = CountVectorizer(min_df=1,max_df=1.0)
X_counts = count_vect.fit_transform(X)
X_counts.shape
tfidf_transformer = TfidfTransformer()
...
conn = pyodbc.connect('Driver={SQL Server};'
                      ...
                      'user =HHVDBCORT01\\TMHBXB78;'
                      'Trusted_Connection=yes;')
sql_query = pd.read_sql_query('SELECT Findings_Value,FINAL_EF FROM NLPFOREF_BANDS_PROSPECTIVE',conn)
df = pd.DataFrame(sql_query, columns=['Findings_Value','FINAL_EF'])
print(df)
df['Findings_Value'] =  df['Findings_Value'].apply(lambda x: re.sub(r'\s+',' ', str(x)))
df['Findings_Value'] =  df['Findings_Value'].apply(lambda x: re.sub(r'\s+[a-z]\s+',' ', str(x)))
...
conn = pyodbc.connect('Driver={SQL Server};'
                      ...
                      'user =HHVDBCORT01\\TMHBXB78;'
                      'Trusted_Connection=yes;')
sql_query = pd.read_sql_query('SELECT * FROM NASIR.DBO.NLPTEXTEF_PROSPECTIVE',conn)
df = pd.DataFrame(sql_query, columns=['Findings_Value','FINAL_EF'])
print(df)
df['Findings_Value'] =  df['Findings_Value'].apply(lambda x: re.sub(r'\s+',' ', str(x)))
df['Findings_Value'] =  df['Findings_Value'].apply(lambda x: re.sub(r'\s+[a-z]\s+',' ', str(x)))
